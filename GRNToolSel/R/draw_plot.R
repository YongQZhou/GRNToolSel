...
#' @param method Receiveroperating characteristic curve, precision-recall curve and sensitivity-specificity curve (roc, prc, ssc). Default: roc.
#' @param algname The algorithm of gene network inferring.
#'
#' @return The curve which selected, such as ROC and RPC.
#' @export
...
draw_plot <- function(dat,method = 'roc',algname){
  ...
  plot_PR <- function(pred,algname)
  {
    ...
    aucpr <- round(aucpr,3)
    PR <- data.frame(Recall = rec, prec = prec, algname = stringr::str_c(algname,' (AUPRC = ', aucpr, ')'))
    palatte <- RColorBrewer::brewer.pal(5, 'Set1')
